...
class Model(object):
    def __init__(self, config):
        ...
        self.crf_layer = config["crf_layer"]
        ...
        # loss of the model
            ...
            if self.model_type == 'bilstm':
                ...
                model_outputs = self.biLSTM_layer(model_inputs, self.lstm_dim, self.lengths)
                # logits for tags
                self.logits = self.softmax_layer_bilstm(model_outputs)
            elif self.model_type == 'idcnn':
                # apply dropout before feed to idcnn layer
                model_inputs = tf.nn.dropout(embedding, self.dropout)
                ...
                # loss of the model
            self.loss = self.loss_layer_softmax(self.logits, self.lengths)
    ...
    def softmax_layer_bilstm(self, lstm_outputs, name=None):
        """
        hidden layer between lstm layer and logits
        :param lstm_outputs: [batch_size, num_steps, emb_size]
        :return: [batch_size, num_steps, num_tags]
    ...
    def loss_layer_softmax(self, project_logits, lengths, name=None):
        """
        calculate Cross Entropy loss
        :param project_logits: [1, num_steps, num_tags]
        :return: scalar loss
        """
        with tf.variable_scope("Cross_Entropy_Loss"  if not name else name):
            self.trans = tf.get_variable(
                "transitions",
                shape=[self.num_tags + 1, self.num_tags + 1],
                initializer=self.initializer)
            cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.targets, logits=project_logits))
            return cost
    ...
    def create_feed_dict(self, is_train, batch):
        ...
        if is_train:
            if self.crf_layer:
                ...
                feed_dict[self.targets] = np_utils.to_categorical(tags,self.num_tags)
                feed_dict[self.dropout] = self.config["dropout_keep"]
        return feed_dict
    ...
    def evaluate(self, sess, data_manager, id_to_tag):
        ...
        fw=codecs.open("trainlog.txt","w","utf-8")
        for res in results:
            fw.write(' '.join(res))
            fw.write("\n")
        return results
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        ...
        wordtags=[]
        for i,char in  enumerate(inputs[0][0]):
            wordtags.append(char+"/"+tags[i])
        return wordtags
